...
async function readStatusFile() {
  await ensureBotFiles();
  const status = await readJsonFile<BotServiceStatusFile>(statusFilePath, buildDefaultStatusFile());
  const normalizedStatus: BotServiceStatusFile = {
    listener: status.listener || buildDefaultStatusFile().listener,
    sender: status.sender || buildDefaultStatusFile().sender,
