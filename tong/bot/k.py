...
def _normalize_epoch_seconds(value):
    """Chuẩn hóa timestamp về giây. Hỗ trợ cả seconds/ms/us/ns."""
    ...
    abs_ts = abs(ts)
    if abs_ts >= 1e18:      # nanoseconds
        ts /= 1_000_000_000.0
    elif abs_ts >= 1e15:    # microseconds
        ts /= 1_000_000.0
    elif abs_ts >= 1e12:    # milliseconds
        ts /= 1000.0

    now = time.time()
    min_valid = 946684800          # 2000-01-01
    max_valid = now + (365 * 24 * 3600)
    if ts < min_valid or ts > max_valid:
        return None

    return ts

def _extract_room_timestamp(*sources):
    """Lấy timestamp hợp lệ đầu tiên từ nhiều nguồn field khác nhau."""
    candidate_fields = (
        "timestamp",
        "posted_at",
        "postedAt",
    ...
    for source in sources:
        ...
        if isinstance(source, dict):
            for field in candidate_fields:
                ts = _normalize_epoch_seconds(source.get(field))
                if ts is not None:
                    return ts
        else:
            ts = _normalize_epoch_seconds(source)
            if ts is not None:
                return ts

    return None

def _is_room_expired(*sources, now=None, retention_seconds=RETENTION_SECONDS):
    """Chỉ đánh dấu hết hạn khi đọc được timestamp hợp lệ."""
    if now is None:
        now = time.time()

    ts = _extract_room_timestamp(*sources)
    if ts is None:
        return False, None

    return ts < (now - retention_seconds), ts
    ...
    if os.path.exists(DISTRICTS_FULL):
        for fname in os.listdir(DISTRICTS_FULL):
            ...
            try:
                ...
                deleted_in_file = 0
                for room in rooms:
                    is_expired, _ = _is_room_expired(room, now=now)
                    if is_expired:
                ...
                if deleted_in_file > 0:
                    print(f"[AUTOCRON] districts_full/{fname}: xóa {deleted_in_file} phòng cũ")
            except Exception as e:
        ...
        for fname in os.listdir(DISTRICTS_OK):
            ...
            try:
                ...
                for room in rooms:
                    ...
                    # Nếu bên full còn dữ liệu thì giữ luôn, tránh xóa nhầm do lệch timestamp.
                    if valid_ids is not None and room_id in valid_ids:
                        kept.append(room)
                        continue

                    # Chỉ xóa khi xác định chắc chắn là đã quá hạn.
                    is_expired, _ = _is_room_expired(room, room_id, now=now)
                    if is_expired:
                        total_deleted_ok += 1
                        continue
