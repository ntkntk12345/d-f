...
class Bot1(ZaloAPI):
    def __init__(self, api_key, secret_key, accounts):
        ...
        ACC2..N (accounts[1:]): Senders (Rotation)
        """
        if len(accounts) < 2:
        ...
        self.send_lock = threading.Lock() # Lock việc gửi khi đang đổi acc (Lock thường là đủ)
        
        # Init Current Sender (ACC2 first)
        self.current_sender = None
        self.current_sender_uid = None
        self.output_groups_map = {} # {keyword: [group_ids]} FOR CURRENT SENDER
        self.group_id_to_name = {}  # {group_id: name} FOR CURRENT SENDER
        ...
        # Stores photos by group_layout_id: {group_layout_id: {"photos": [...], "symbol": "...", "source_id": "...", "author_id": "...", "timestamp": ...}}
        self.photo_cache = {}
        self.photo_cache_lock = threading.RLock() # Sử dụng RLock cho đồng bộ
        ...
        # Deduplication toàn cục cho nội dung (Tránh cùng 1 tin từ 11A, 12A gửi đi 2 lần)
        ...
        self._init_current_sender()
        
        # Rotation Thread (1h đổi acc)
        ...
        self.send_thread = threading.Thread(target=self._send_worker_sequential, daemon=True)
        ...
        print(f"[BOT1] ACC1 = Listen")
        print(f"[BOT1] Rotation Mode: 1 Active Sender. Switch every 1h or on Error 221")

    def _rotate_account(self):
        """Chuyển sang tài khoản tiếp theo trong vòng tròn"""
        with self.send_lock: # Stop sending while switching
            print(f"\n[ROTATION] ⏳ Initiating account rotation...")
            next_index = (self.current_sender_index + 1) % len(self.sender_accounts_config)
            self.current_sender_index = next_index
            self._init_current_sender()
    
    def _rotation_monitor(self):
        """Thread quản lý vòng đời xoay tua 1 tiếng"""
        print("[ROTATION] Started. Cycle: 1 Hour.")
        while self.is_running:
            ...
            print("[ROTATION] ⏰ Time to switch account.")
            
            # 2. Wait for Queue Draining
            while not self.send_queue.empty():
                print(f"[ROTATION] ⏳ Waiting for queue to empty ({self.send_queue.qsize()} pending)...")
                time.sleep(5)
                if not self.is_running: return
    ...
    def _init_current_sender(self):
        """Khởi tạo/Switch sang tài khoản gửi hiện tại (self.current_sender_index)"""
        acc_cfg = self.sender_accounts_config[self.current_sender_index]
        print(f"\n[ROTATION] 🔄 Switching to Sender #{self.current_sender_index + 1}...")
        
        try:
            # 1. Dispose old sender if any
            if self.current_sender:
                # cleanup connection if needed? ZaloAPI mostly stateless or thread local
                self.current_sender = None
                
            # 2. Init New Sender
            self.current_sender = ZaloAPI(self.api_key, self.secret_key, 
                                          imei=acc_cfg["imei"], 
                                          session_cookies=acc_cfg["session_cookies"])
            self.current_sender.name = f"Sender-{self.current_sender_index+1}"
            
            try:
                self.current_sender_uid = str(self.current_sender._state.user_id)
            except:
                self.current_sender_uid = "N/A"
                
            print(f"[ROTATION] ✓ Logged in {self.current_sender.name} (UID: {self.current_sender_uid})")
            
            # 3. SCAN GROUPS for THIS Sender
            self.output_groups_map = {} # Reset output map
            self.group_id_to_name = {}  # Reset name cache
            
            print(f"[ROTATION] Đang quét nhóm Output cho {self.current_sender.name}...")
            all_groups = self.current_sender.fetchAllGroups()
            group_ids = []
            grid_map = {}
            ...
            # Nếu cache thiếu, fetch thêm (tối đa 200 nhóm để tiết kiệm thời gian)
            if len(grid_map) < len(group_ids):
                missing = [g for g in group_ids if str(g) not in grid_map]
                if missing:
                    ...
                    for i in range(0, min(len(missing), 200), chunk_size):
                        ...
                        try:
                             batch = {str(gid): 0 for gid in chunk}
                             res = self.current_sender.fetchGroupInfo(batch)
            ...
            for gid_str, data in grid_map.items():
                ...
                self.group_id_to_name[gid_str] = name
                ...
                if matched:
                    ...
                    for kw in matched:
                        kw = kw.lower()
                        if kw not in self.output_groups_map:
                            self.output_groups_map[kw] = []
                        # CRITICAL FIX: Don't add duplicate group_id
                        if gid_str not in self.output_groups_map[kw]:
                            self.output_groups_map[kw].append(gid_str)
            
            print(f"[ROTATION] ✓ Đã map {count_avail} nhóm Output khả dụng cho Session này.")
            
        except Exception as e:
            print(f"[ROTATION] ✗ Error init sender: {e}")
            # Nếu lỗi init, có thể switch tiếp luôn?
            # self._rotate_account() # Cẩn thận infinite recurse
            pass
    ...
    def _handle_send_error(self, e):
        """Check lỗi 221 để trigger rotation"""
        if "221" in str(e):
            return True
        return False

    def _send_worker_sequential(self):
        """
        Worker gửi tin tuần tự sử dụng Current Sender.
        Xử lý lỗi 221 -> Đổi acc ngay lập tức.
        """
        print("[SENDER] 🚀 Sequential Worker started.")
        while self.is_running:
            try:
                ...
                # Resolve Keywords -> Group IDs (Current Sender Context)
                target_groups = set()
                with self.send_lock: 
                     for kw in keywords:
                         gids = self.output_groups_map.get(kw.lower(), [])
                         target_groups.update(gids)
                
                target_groups = list(target_groups)
                
                if not target_groups:
                    print(f"[SENDER] ⚠️ Skip: {source_info} -> No matching groups for {self.current_sender.name}")
                    self._finish_current_task() # Mark done even if skipped
                    self.send_queue.task_done()
                    continue
                
                # DEBUG: Log matched keywords and groups
                print(f"[SENDER] � Matched keywords: {keywords}")
                print(f"[SENDER] �🚀 {self.current_sender.name} sending {source_info} -> {len(target_groups)} UNIQUE groups")
                
                # Send Loop
                should_rotate = False
                
                # Get already sent groups for this session (from persistence)
                already_sent = set(item.get("sent_groups", []))
                
                for i, group_id in enumerate(target_groups):
                    # Check if already sent to this group (in case of retry/crash recovery)
                    if group_id in already_sent:
                        print(f"[SENDER] ⏩ Skip {group_id} (Already sent in this session)")
                        continue
                    # "mỗi lần đổi acc là phải quét lại các nhóm ... k sài id của acc cũ"
                    # critical: Group IDs are specific to the sender account? 
                    # Usually Zalo group IDs are global (Grid ID). But let's assume valid.
                    # BUT: if "mỗi tài khoản 1 id nhóm khác nhau" means the LIST of groups is different.
                    # Then if I switch sender, I must RE-RESOLVE keywords to new group IDs.
                    
                    if should_rotate:
                        self._rotate_account()
                        # Re-resolve groups for new sender?
                        # This task is halfway done. 
                        # To keep it simple: If error 221, we abort this task for the remaining groups 
                        # and let the queue pick up NEXT item with new sender.
                        # This avoids sending duplicate to same group if we re-process.
                        # User said "gửi các tin nhắn xếp hàng" (queue).
                        break
                    ...
                    with self.send_lock: 
                        ...
                        group_name = self.group_id_to_name.get(group_id, group_id)
                        
                        try:
                            # 0. Send OPENING STICKER
                            try:
                                sender.sendSticker(3, 50625, 12658, group_id, ThreadType.GROUP)
                                print(f"[SEND] ✓ Opening Sticker → {group_name}")
                                time.sleep(random.uniform(0.5, 1.0))
                            except:
                                pass
                            
                            # 1. Build chronological timeline of all content
                            timeline = []
                            ...
                            # Sort by timestamp
                            # CONDITIONAL: Only 11A/12A need Text First forcing. Others respect timestamp.
                            def sort_key(x):
                                if "11A" in symbol or "12A" in symbol:
                                    priority = 0 if x["type"] == "text" else 1
                                    return (priority, x["timestamp"])
                                return (0, x["timestamp"]) # Default chronological
                            
                            timeline.sort(key=sort_key)
                            
                            # 2. Send in chronological order, batching consecutive photos
                            photo_batch = []
                            ...
                            for item in timeline:
                                if item["type"] == "text":
                                    # Flush photos first
                                    if photo_batch:
                                        self._send_photos_multisender(sender, photo_batch, group_id)
                                        photo_batch = []
                                        time.sleep(random.uniform(1.0, 2.0))
                                    
                                    # Send text
                                    sender.send(Message(text=item["data"]), group_id, ThreadType.GROUP)
                                    print(f"[SEND] ✓ Text ({symbol}) → {group_name}")
                                    time.sleep(random.uniform(1.0, 2.0))
                                
                                elif item["type"] == "photo":
                                    photo_batch.append(item["data"])
                                    ...
                                    stk = item["data"]
                                    try:
                                        sender.sendSticker(stk.get("type", 3), stk.get("id"), stk.get("catId"), group_id, ThreadType.GROUP)
                            ...
                            # Update PROGRESS (Sent Groups)
                            try:
                                if "sent_groups" not in item: item["sent_groups"] = []
                            ...
                            if self._handle_send_error(e):
                                print(f"[SENDER] 🛑 SPAM LIMIT (221). Rotating...")
                                should_rotate = True
                
                # Removing from Persistence (First item in file)
                print(f"[PERSISTENCE] Xóa task khỏi queue file...")
                self._finish_current_task()
                self.send_queue.task_done()
                
                # If we rotated, maybe wait a bit
                if should_rotate:
                    time.sleep(5)
                else:
                ...
                # Important: Mark task as done even on error to prevent infinite restart loops
                self._finish_current_task()
                try: 
                    self.send_queue.task_done()
                except: pass
    ...
    def onMessage(self, mid, author_id, message, message_object, thread_id, thread_type):
        ...
        try:
            ...
            # 1. Bỏ qua tin nhắn từ chính bot (ACC2 - Sender)
            if self.current_sender_uid and author_id_str == self.current_sender_uid:
                return
            ...
            if thread_id_str in self.group_id_to_name:
                return
            ...
            if isinstance(message, str) and message.strip().lower() == "!sticker":
                ...
                self.current_sender.sendSticker(3, 50625, 12658, thread_id_str, ThreadType.GROUP)
            ...
            if isinstance(message, str) and message.strip().lower() == "!help":
                ...
                self.current_sender.send(Message(text=help_text), thread_id_str, ThreadType.GROUP)
            ...
            group_name = self.group_id_to_name.get(thread_id_str, thread_id_str)
