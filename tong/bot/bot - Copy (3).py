...
NTK_FILE = "ntk.txt"  # File chứa tên nhóm tổng NTK
...
class Client(ZaloAPI):
    def __init__(self, api_key, secret_key, imei, session_cookies):
        ...
        self.is_listening = False  # Flag để track xem đang listen không
        ...
        self.ntk_group_name = None  # Tên nhóm tổng NTK từ file ntk.txt
        self.ntk_group_id = None  # ID nhóm tổng NTK (resolve sau khi build cache)
        ...
        self.session_timeout = 15.0  # 15 giây timeout cho session (tăng để đợi ảnh đầy đủ)
        ...
        self.route_sessions = {}  # {(thread_id, author_id): {"id": int, "updated_at": float}}
        self.last_route_context = {}  # {(thread_id, author_id): {...}}
        self.media_batch_context = {}  # {batch_key: batch_data}
    ...
    def _load_routing_config(self):
        ...
        # Load nhóm tổng NTK từ file ntk.txt
        ntk_lines = self._load_lines(NTK_FILE)
        if ntk_lines:
            self.ntk_group_name = ntk_lines[0].strip()
            print(f"[CONFIG] Đã load nhóm tổng NTK: '{self.ntk_group_name}'")
        else:
            self.ntk_group_name = None
            print(f"[CONFIG] Không tìm thấy nhóm tổng NTK trong {NTK_FILE}")

    def _resolve_ntk_group_id(self):
        """Resolve ID nhóm tổng NTK từ tên đã load."""
        if not self.ntk_group_name:
            return None
        
        if self.ntk_group_id:
            return self.ntk_group_id
        
        # Thử resolve từ cache
        resolved_id = self._resolve_output_thread_id(self.ntk_group_name)
        if resolved_id:
            self.ntk_group_id = resolved_id
            print(f"[NTK] Đã resolve nhóm tổng NTK '{self.ntk_group_name}' → {self.ntk_group_id}")
        else:
            print(f"[NTK] Không thể resolve nhóm tổng NTK '{self.ntk_group_name}'")
    ...
    def _build_group_name_cache(self):
        ...
        try:
            ...
            # Resolve nhóm tổng NTK sau khi có cache
            if self.ntk_group_name:
                self._resolve_ntk_group_id()
        except Exception as e:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if task_type == "text":
                        # Gửi text (đã format: thêm tag và xóa emoji)
                        # Nếu text >= 25 ký tự thì gửi sticker trước
                        try:
                            ...
                            if formatted_text:  # Chỉ gửi nếu còn text sau khi format
                                # Kiểm tra độ dài text (sau khi format)
                                text_length = len(formatted_text)
                                if text_length >= 25:
                                    # Gửi sticker trước nếu text >= 25 ký tự
                                    try:
                                        self._send_sticker(dest_id)
                                        print(f"[QUEUE] Đã gửi sticker trước text dài ({text_length} ký tự) → {dest_id}")
                                        time.sleep(1)  # Delay 1 giây sau sticker
                                    except Exception as sticker_e:
                                        print(f"[QUEUE] Lỗi gửi sticker: {sticker_e}")
                                
                                msg = Message(text=formatted_text)
                                self.send(msg, dest_id, ThreadType.GROUP)
                                print(f"[QUEUE] Đã gửi text ({text_length} ký tự) từ {source_group_id or '?'} → {dest_id} (tag: {self._get_input_tag(source_group_id, source_group_name) or 'không có'})")
    ...
    def _format_text(self, text, source_group_id=None, group_name=None):
        ...
        Tag được thêm SAU khi xóa emoji để đảm bảo tag không bị xóa mất.
        """
        if not text:
            return text
        ...
        # Thêm tag vào đầu text SAU khi đã xóa emoji và format (để đảm bảo tag không bị xóa)
        if tag:
            text = f"{tag} {text}"
    ...
    def _get_destination_ids(self, primary_dest_id):
        """
        Lấy danh sách tất cả các đích gửi (gồm dest_id chính và nhóm tổng NTK).
        Sử dụng đa luồng để gửi song song cho nhanh.
        """
        dest_ids = set()
        
        if primary_dest_id:
            dest_ids.add(str(primary_dest_id))
        
        # Thêm nhóm tổng NTK nếu có
        ntk_id = self._resolve_ntk_group_id()
        if ntk_id and str(ntk_id) not in dest_ids:
            dest_ids.add(str(ntk_id))
            print(f"[NTK] Sẽ gửi tới cả nhóm tổng NTK: {ntk_id}")
        
        return list(dest_ids)
    
    def _enqueue_to_multiple_dests(self, dest_ids, source_group_id, text, photos, videos, source_group_name=None):
        """Đưa message vào queue cho nhiều đích cùng lúc (đa luồng)."""
        for dest_id in dest_ids:
            if dest_id:
                self._enqueue_message(dest_id, source_group_id, text, photos, videos, source_group_name)
    ...
    def _flush_current_session(self, group_id, user_id):
        ...
        try:
            ...
            # Lấy tất cả đích gửi (bao gồm nhóm tổng NTK)
            destination_ids = self._get_destination_ids(current_dest_id)
            print(f"[SESSION] ===== FLUSH SESSION ===== {buffer_key}: keyword='{current_keyword}', {len(session_texts)} texts, {photo_count} photos, {video_count} videos → {len(destination_ids)} đích: {destination_ids}")
            ...
            while i < len(all_items_sorted):
                ...
                if item["type"] == "text":
                    # Gửi text riêng - tìm group_name từ cache để đảm bảo có tag
                    source_group_name = None
                    if group_id:
                        # Tìm group_name từ cache
                        group_id_str = str(group_id)
                        for name, gid in self.group_name_cache.items():
                            if str(gid) == group_id_str:
                                # Tìm tên gốc (không normalized)
                                for raw_name in self.input_groups:
                                    if raw_name.lower() == name or self._normalize_for_match(raw_name) == name:
                                        source_group_name = raw_name
                    ...
                    self._enqueue_to_multiple_dests(
                        destination_ids,
                        group_id,
                        item["text"],
                        [],
            ...
            with lock:
                ...
                if msg_type == "chat.sticker":
                    ...
                    if not current_keyword or not current_dest_id:
                        ...
                        # Nếu session vừa bị flush gần đây (trong vòng 15 giây) → có thể ảnh thuộc session đó
                        # Tạo lại session tạm thời để nhận ảnh (sẽ được flush lại sau)
                        elif last_flush_time > 0 and time_since_flush < 15.0:
                    ...
                    if keyword:
                        ...
                        if dest_id:
                            # Kiểm tra xem có session cũ cần flush không
                            # Flush nếu: có session cũ với dữ liệu (dù keyword giống hay khác)
                            has_old_session = current_keyword and current_dest_id
                            has_old_data = False
                            old_session_data = None
                            if has_old_session:
                                old_texts = buffer.get("session_texts", [])
                                old_media = buffer.get("session_media", [])
                                has_old_data = len(old_texts) > 0 or len(old_media) > 0
                                if has_old_data:
                                    # Lưu lại dữ liệu session cũ để flush sau
                                    old_session_data = {
                                        "keyword": current_keyword,
                                        "dest_id": current_dest_id,
                                        "texts": old_texts.copy(),
                                        "media": old_media.copy(),
                                    }
                            ...
                            should_send_sticker = last_keyword_for_dest != keyword
                            
                            # Nếu có session cũ với dữ liệu → flush session cũ trước (ngoài lock)
                            need_flush_old = has_old_session and has_old_data
                            
                            if need_flush_old:
                                if current_keyword != keyword:
                                    print(f"[SESSION] Keyword mới '{keyword}' khác keyword cũ '{current_keyword}', sẽ flush session cũ")
                                else:
                                    print(f"[SESSION] Keyword '{keyword}' xuất hiện lại (cùng keyword), sẽ flush session cũ trước khi bắt đầu session mới")
                            ...
                            # Lưu lại để flush và gửi sticker sau khi release lock
                            flush_old_data = old_session_data if need_flush_old else None
                            send_sticker_after_lock = should_send_sticker
                            sticker_dest_id = dest_id if send_sticker_after_lock else None
                        else:
                            print(f"[SESSION] Tìm thấy keyword '{keyword}' nhưng không resolve được dest_id")
                            flush_after_lock = False
                            send_sticker_after_lock = False
                            sticker_dest_id = None
                    else:
                ...
                # Lưu lại biến để dùng sau khi ra khỏi lock
                flush_old_data_after = flush_old_data if 'flush_old_data' in locals() else None
                send_sticker_after = send_sticker_after_lock if 'send_sticker_after_lock' in locals() else False
                sticker_dest_after = sticker_dest_id if 'sticker_dest_id' in locals() else None
                keyword_for_sticker = keyword if 'keyword' in locals() else None
            
            # Sau khi ra khỏi lock, flush session cũ và gửi sticker
            if flush_old_data_after:
                # Flush dữ liệu session cũ đã lưu
                print(f"[SESSION] Flush session cũ trước khi bắt đầu session mới...")
                try:
                    old_keyword = flush_old_data_after["keyword"]
                    old_dest_id = flush_old_data_after["dest_id"]
                    old_texts = flush_old_data_after["texts"]
                    old_media = flush_old_data_after["media"]
                    
                    # Gửi dữ liệu session cũ - dùng logic tương tự _flush_current_session
                    if old_texts or old_media:
                        # Tạo danh sách tất cả items (text và media) và sắp xếp theo timestamp
                        all_items = []
                        for text_item in old_texts:
                            all_items.append({
                                "type": "text",
                                "text": text_item.get("text", ""),
                                "timestamp": text_item.get("timestamp", 0),
                        ...
                        # Lấy tất cả đích gửi (bao gồm nhóm tổng NTK)
                        old_destination_ids = self._get_destination_ids(old_dest_id)
                        ...
                        photo_count = len([m for m in old_media if m.get('type') == 'photo'])
                        video_count = len([m for m in old_media if m.get('type') == 'video'])
                        print(f"[SESSION] Đã flush session cũ: keyword='{old_keyword}', {len(old_texts)} texts, {photo_count} photos, {video_count} videos → {len(old_destination_ids)} đích")
                except Exception as flush_e:
                    print(f"[SESSION] Lỗi flush session cũ: {flush_e}")
                    import traceback
                    print(f"[SESSION] Traceback: {traceback.format_exc()}")
            
            # Bỏ gửi sticker - không gửi sticker trước mỗi keyword mới
        # if send_sticker_after and sticker_dest_after and keyword_for_sticker:
        #     # Gửi sticker cho keyword mới
        #     print(f"[STICKER] Keyword mới '{keyword_for_sticker}' cho dest_id {sticker_dest_after}, gửi sticker")
        #     try:
        #         self._send_sticker(sticker_dest_after)
        #         self.last_keyword_per_dest[sticker_dest_after] = keyword_for_sticker
        #     except Exception as e:
        #         print(f"[STICKER] Lỗi gửi sticker khi keyword mới: {e}")
        
            # Vẫn cập nhật last_keyword_per_dest để track keyword
            if send_sticker_after and sticker_dest_after and keyword_for_sticker:
                self.last_keyword_per_dest[sticker_dest_after] = keyword_for_sticker
            
            # Tạo timeout timer mới (10s) - sau khi flush session cũ và gửi sticker
            try:
                buffer, lock = self._get_or_create_buffer(thread_id, author_id)
                with lock:
                ...
                print(f"[BUFFER] Lỗi tạo timeout timer: {timer_e}")
...
if __name__ == "__main__":
    ...
    while True:  # Restart vô hạn, không dừng
        try:
            ...
            try:
                # Đợi một chút trước khi listen để tránh restart quá nhanh
                time.sleep(2)
                
                # Kiểm tra xem đã đang listen chưa
                if client.is_listening:
                    print(f"[MAIN] Đang listen rồi, bỏ qua...")
                    time.sleep(10)
                    continue
                
                print(f"[MAIN] Bắt đầu listen...")
                
                # Thử đóng connection cũ nếu có (nếu ZaloAPI có method stop)
                try:
                    if hasattr(client, 'stop'):
                        client.stop()
                        print(f"[MAIN] Đã đóng connection cũ (nếu có)")
                        time.sleep(1)
                except Exception as stop_e:
                    pass  # Bỏ qua nếu không có method stop
                
                # Đánh dấu đang listen
                client.is_listening = True
                
                # listen() với thread=True sẽ chạy trong background thread
                # Nó sẽ block cho đến khi có lỗi hoặc connection đóng
                client.listen(thread=True, delay=0)
                
                # Nếu listen() return, reset flag
                client.is_listening = False
                
                # Nếu listen() return ngay (không có exception), có thể do warning "Another connection"
                # Đợi một chút rồi check xem có thực sự cần restart không
                print(f"[MAIN] listen() đã return, đợi 5s để check connection...")
                time.sleep(5)
                
                # Kiểm tra lại connection trước khi restart
                try:
                    test_groups = client.fetchAllGroups()
                    if test_groups:
                        print(f"[MAIN] Connection vẫn OK sau khi listen() return, tiếp tục listen...")
                        # Đợi thêm một chút trước khi listen lại để tránh conflict
                        time.sleep(3)
                        continue  # Tiếp tục listen, không restart
                    else:
                        print(f"[MAIN] Connection không OK, restart...")
                except Exception as check_e:
                    print(f"[MAIN] Lỗi kiểm tra connection: {check_e}, restart...")
            except Exception as listen_e:
                # Reset flag khi có lỗi
                client.is_listening = False
                
                error_msg = str(listen_e)
                # Bỏ qua warning "Another connection is opened" - không restart
                if "Another connection" in error_msg or "connection" in error_msg.lower():
                    print(f"[MAIN] Warning về connection (có thể là 'Another connection is opened'), đợi 5s rồi tiếp tục...")
                    time.sleep(5)
                    # Kiểm tra connection trước khi tiếp tục
                    try:
                        test_groups = client.fetchAllGroups()
                        if test_groups:
                            print(f"[MAIN] Connection vẫn OK, tiếp tục listen...")
                            time.sleep(3)
                            continue
                        else:
                            print(f"[MAIN] Connection không OK sau warning, restart...")
                    except Exception:
                        print(f"[MAIN] Không kiểm tra được connection, restart...")
                    # Nếu không continue được thì sẽ restart
                
                print(f"[MAIN] Lỗi trong listen: {listen_e}")
                import traceback
                print(f"[MAIN] Traceback: {traceback.format_exc()}")
                # Đợi một chút trước khi restart để tránh loop
                time.sleep(5)
            ...
            # Tăng thời gian chờ nếu lỗi liên tiếp (tối thiểu 10 giây để tránh restart quá nhanh)
            wait_time = max(10, min(5 + consecutive_failures * 2, 30))  # Tối thiểu 10s, tối đa 30 giây
