...
class NTKBot(ZaloAPI):
    def __init__(self, api_key, secret_key, imei, session_cookies):
        ...
        self.openrouter_api_key = "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"
    ...
    def _load_rooms_for_ai(self):
        ...
        try:
            ...
            # Cache room theo ID gốc (ID dài)
            self.loaded_rooms_cache = {r.get("id"): r for r in rooms if r.get("id")}
            
            # Tạo mapping: số ngắn (1, 2, 3...) -> ID gốc dài
            # Ví dụ: "7" -> "room_1766958272301_4255"
            self.room_id_map = {}
            
            # Format rõ ràng, dễ đọc cho AI
            # Mỗi phòng có đường kẻ phân cách
            rooms_text = "📋 DANH SÁCH PHÒNG:\n\n"
            for i, room in enumerate(rooms, 1):
                r_id_original = room.get("id", "N/A")
                short_id = str(i)
                
                # Lưu mapping: số ngắn -> ID gốc
                self.room_id_map[short_id] = r_id_original
                ...
                # Format rõ ràng, không dùng đường kẻ
                rooms_text += f"🏠 CĂN {short_id}\n"
                rooms_text += f"📍 Địa chỉ: {addr}\n"
                rooms_text += f"💰 Giá: {price} triệu/tháng\n"
                rooms_text += f"📝 Mô tả: {desc}...\n\n"
...
📌 Về dữ liệu:
• Chỉ tư vấn phòng có trong danh sách trên
• Gọi tên phòng theo "Căn [số]" (ví dụ: "Căn 1", "Căn 7", "Căn 15")

📌 Về định dạng câu trả lời:
• LUÔN LUÔN xuống dòng sau mỗi phần thông tin
• Sử dụng emoji để dễ nhìn (🏠 📍 💰 📝)
• MỖI PHÒNG cách nhau bằng 1 dòng trống
• KHÔNG viết liền quá 3 dòng
• KHÔNG dùng đường kẻ (━━━)

📌 Về hướng dẫn xem ảnh:
• Nếu khách quan tâm, bảo: "Gõ !showanh [số] để xem ảnh"
• Ví dụ: "Gõ !showanh 7 để xem ảnh Căn 7 nhé"

📌 Tự động hiển thị ảnh:
• Nếu khách hỏi: "cho tao xem căn X", "xem căn X", "show căn X"
• Trả lời: "Bạn muốn xem Căn X đúng không? Đang gửi ảnh...

[AUTO_SHOWANH:X]"
• X là số thứ tự căn (1, 2, 3...)
"""
    ...
    def _auto_execute_showanh(self, display_id, thread_id, thread_type):
        ...
        try:
            ...
            # Get room data
            room = self.loaded_rooms_cache.get(original_id)
            
            if not room:
                self.send(Message(text=f"❌ Lỗi: Không tìm thấy dữ liệu phòng"), thread_id, thread_type)
                print(f"[AUTO_SHOWANH] Không tìm thấy room với original_id {original_id}")
                return
            
            print(f"[AUTO_SHOWANH] Đang gửi thông tin phòng {original_id} (display_id: {display_id})")
            ...
            if photos:
                self.send(Message(text=f"📸 Đang gửi {len(photos)} ảnh..."), thread_id, thread_type)
                # Chạy gửi ảnh trong thread riêng để ko block
                threading.Thread(target=self._send_photos_grouped, args=(photos, thread_id, thread_type), daemon=True).start()
                print(f"[AUTO_SHOWANH] Đã gửi {len(photos)} ảnh")
            else:
                self.send(Message(text=f"⚠️ Phòng này không có ảnh."), thread_id, thread_type)
                print(f"[AUTO_SHOWANH] Phòng {r_id} không có ảnh")
                
        except Exception as e:
            print(f"[AUTO_SHOWANH] Lỗi: {e}")
            import traceback
            print(f"[AUTO_SHOWANH] Traceback: {traceback.format_exc()}")
            self.send(Message(text=f"❌ Lỗi khi hiển thị phòng: {e}"), thread_id, thread_type)
                ...
                # Command: !showanh <id/số thứ tự>
                    ...
                    if match:
                        ...
                        try:
                            ...
                            if not hasattr(self, 'loaded_rooms_cache') or not hasattr(self, 'room_id_map'):
                                self._load_rooms_for_ai()
                            
                            # 1. Thử tìm theo số ngắn (1, 2, 3...) -> chuyển sang ID gốc
                            original_id = self.room_id_map.get(input_id)
                            
                            # 2. Nếu không thấy trong map, coi input_id là ID gốc (fallback)
                            if not original_id:
                                original_id = input_id
                            
                            # 3. Tìm room theo ID gốc
                            room = self.loaded_rooms_cache.get(original_id)
                ...
                if self.system_prompt and message and isinstance(message, str):
                    ...
                    def handle_ai_reply():
                        try:
                            ...
                            if ai_reply:
                                # Check for auto-command pattern
                                import re
                                auto_cmd_match = re.search(r'\[AUTO_SHOWANH:(\d+)\]', ai_reply)
                                
                                if auto_cmd_match:
                                    display_id = auto_cmd_match.group(1)
                                    print(f"[AI] Phát hiện AUTO_SHOWANH với display_id: {display_id}")
                                    
                                    # Remove the command tag from visible text
                                    clean_reply = re.sub(r'\[AUTO_SHOWANH:\d+\]', '', ai_reply).strip()
                                    ...
                                    # Auto-execute !showanh
                                    self._auto_execute_showanh(display_id, thread_id_str, thread_type)
                                    
                                    # Forward câu trả lời của AI cho Admin (bao gồm tag để admin biết)
                                    self.send(Message(text=f"🤖 [AI Reply + AUTO]: {ai_reply}"), dest_id, ThreadType.GROUP)
                                else:
