...
class ZaloAPI(object):
    ...
    def _uploadImage(self, filePath, thread_id, thread_type):
        ...
        if results:
            ...
            print(f"[DEBUG_UPLOAD] Response keys: {list(results.keys()) if isinstance(results, dict) else results}")
            print(f"[DEBUG_UPLOAD] Full response: {results}")
        ...
        if custom_payload:
            ...
            payload = {
                "params": {
                    ...
                    "width": width,
                    "height": height,
                    "rawUrl": uploadImage.get("normalUrl") or uploadImage.get("url") or "",
            ...
            if thread_type == ThreadType.USER:
                url = "https://tt-files-wpa.chat.zalo.me/api/message/photo_original/send"
                payload["params"]["toid"] = str(thread_id)
                payload["params"]["normalUrl"] = uploadImage.get("normalUrl") or uploadImage.get("url") or ""
            elif thread_type == ThreadType.GROUP:
                url = "https://tt-files-wpa.chat.zalo.me/api/group/photo_original/send"
                payload["params"]["grid"] = str(thread_id)
                payload["params"]["oriUrl"] = uploadImage.get("normalUrl") or uploadImage.get("url") or ""
            else:
                raise ZaloUserError("Thread type is invalid")
        ...
        with connect(url, additional_headers=headers) as ws:
            ...
            while not self._condition.is_set():
                try:
                    ...
                    elif n == 1 and cmd in [502, 522] and s == 0:
                        # Delivereds, Seen, Clear Unread, ...
                        continue

                    elif n == 1 and cmd in [504, 524] and s == 0:
                        # Undo / Recall handling
                        try:
                            parsedData = _util.zws_decode(parsed, self.ws_key)
                            undo_data = parsedData.get("data", {})
                            
                            if cmd == 504: # User Undo
                                msgs = undo_data.get("msgs", [])
                                for m in msgs:
                                    msgId = m.get("msgId")
                                    # Identify thread (sender)
                                    sender = str(int(m.get("uidFrom") or "0"))
                                    if msgId:
                                        if thread: pool.submit(self.onMessageUndo, msgId, sender, ThreadType.USER)
                                        else: self.onMessageUndo(msgId, sender, ThreadType.USER)
                                
                            elif cmd == 524: # Group Undo
                                groupMsgs = undo_data.get("groupMsgs", [])
                                for m in groupMsgs:
                                    msgId = m.get("msgId")
                                    groupId = str(int(m.get("idTo") or "0"))
                                    if msgId:
                                        if thread: pool.submit(self.onMessageUndo, msgId, groupId, ThreadType.GROUP)
                                        else: self.onMessageUndo(msgId, groupId, ThreadType.GROUP)
                        except Exception as e:
    ...
    def onLoggingIn(self, phone=None):
        ...
        """
        logger.debug("Light Đang Loging Vào Acc {}...".format(phone))
    ...
    def onMessageUndo(
        ...
        """Called when a message is undone/recalled.
        
        Args:
           msg_id: ID of the recalled message
           thread_id: Thread ID
           thread_type: Thread Type
        """
        logger.info("Message {} undone in {}".format(msg_id, thread_id))
