        ...
        def heartbeat_worker():
            ...
            while self.is_running:
                try:
                    ...
                    queues_active = sum(1 for q in self.send_queues.values() if not q.empty())
                    buffers_active = len([b for b in self.buffers.values() if b.get("session_texts") or b.get("session_media")])
                    ...
                    if time_since_last > 300:  # 5 phút không có tin nhắn
                        if not restart_pending:
                            ...
                            if queues_active > 0 or buffers_active > 0:
                                # Có tin đang xử lý → reset đợi
                                print(f"[HEARTBEAT] Có {queues_active} queue và {buffers_active} buffer đang xử lý, đợi...")
            ...
            print(f"[HEARTBEAT] Heartbeat thread kết thúc")
                ...
                if msg_type == "chat.sticker":
                    ...
                    if keyword:
                        ...
                        if dest_id:
                            ...
                            # Có keyword nhưng không resolve được dest_id → gửi vào nhóm tổng NTK
                            ntk_id = self._resolve_ntk_group_id()
                            if ntk_id:
                                print(f"[SESSION] Keyword '{keyword}' không có nhóm đầu ra, gửi vào nhóm tổng NTK: {ntk_id}")
                                # Tạo session gửi vào nhóm tổng
                                buffer["current_keyword"] = keyword
                                buffer["current_dest_id"] = ntk_id
                                buffer["session_start_time"] = time.time()
                                buffer["session_texts"] = [{"text": text_to_check, "timestamp": time.time()}]
                                buffer["session_media"] = []
                                buffer["is_flushing"] = False
                                ...
                                print(f"[SESSION] Keyword '{keyword}' không có nhóm đầu ra VÀ không có nhóm tổng, bỏ qua")
                        ...
                        if current_keyword and current_dest_id:
                            ...
                            # Chưa có session và không có keyword → kiểm tra nhóm tổng NTK
                            ntk_id = self._resolve_ntk_group_id()
                            if ntk_id:
                                # Có nhóm tổng NTK → tạo session gửi vào nhóm tổng
                                buffer["current_keyword"] = "__NTK_ONLY__"  # Đánh dấu session chỉ gửi NTK
                                buffer["current_dest_id"] = ntk_id
                                buffer["session_start_time"] = time.time()
                                buffer["session_texts"] = [{"text": text_to_check, "timestamp": time.time()}]
                                buffer["session_media"] = []
                                ...
                                buffer["is_flushed"] = False
                                if is_long_text:
                                    print(f"[SESSION] Text dài ({text_length} ký tự) không có keyword, tạo session gửi vào nhóm tổng NTK: {ntk_id}")
                                else:
                                    print(f"[SESSION] Text không có keyword, tạo session gửi vào nhóm tổng NTK: {ntk_id}")
                            else:
                                # Không có nhóm tổng NTK → bỏ qua
                                if is_long_text:
                                    print(f"[BUFFER] Text dài ({text_length} ký tự) không có keyword và không có nhóm tổng, bỏ qua")
                                else:
                                    print(f"[BUFFER] Text không có keyword và không có nhóm tổng, bỏ qua")
                                return
                        flush_after_lock = False
                        send_sticker_after_lock = False
                        sticker_dest_id = None
...
if __name__ == "__main__":
    ...
    while True:  # Restart vô hạn
        ...
        try:
            ...
            # listen() trả về ngay, nhưng bot sẽ chạy trong background
            # Đợi cho đến khi heartbeat set is_running = False
            while client.is_running:
                time.sleep(1)
            
            # is_running = False => cần restart
            print(f"[MAIN] 🔄 Bot cần restart, đang cleanup...")
            ...
            print(f"[MAIN] Đợi 5 giây trước khi restart...")
            time.sleep(5)
